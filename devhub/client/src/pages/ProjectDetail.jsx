...
import {
  ...
  CheckCircle, Circle, Plus, Upload, FileText, Trash2
} from 'lucide-react';
...
export default function ProjectDetail() {
  ...
  const [deletingFile,      setDeletingFile]      = useState(null); // publicId being deleted
  ...
  const handleDeleteFile = async (file) => {
    ...
    try {
      const encoded = encodeURIComponent(file.publicId);
      const res = await api.delete(`/projects/${id}/files/${encoded}`);
      setProject(res.data);
    } catch (err) {
          ...
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={saving}
                className="text-xs bg-amber-500 text-white px-2 py-0.5 rounded-md hover:bg-amber-400 transition-colors disabled:opacity-60">
                {saving ? 'Saving...' : 'Save'}
              </button>
            </div>
          </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              disabled={uploading}
              className="flex items-center gap-1.5 text-sm text-amber-500 hover:text-amber-600 font-medium transition-colors disabled:opacity-60">
              {uploading
            ...
            {(project.deliveryFiles || []).map((f) => (
              <div
                ...
                <button
                  ...
                  disabled={deletingFile === f.publicId}
